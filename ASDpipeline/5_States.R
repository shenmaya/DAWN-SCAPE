...
# Classify Clusters ----
#'#######################################################################
etiol_clusts <- c(1, 4, 5, 6)
emerg_clusts <- c(2, 9, 10, 11, 12, 14)
other_clusts <- c(3, 7, 8, 13)
...
clust_groups$clust_order <- clust_order
...
for (i in 1:11) {
  ...
  if ((clust_state_mx_scl_bystate[1, i] > 0.15) | (clust_state_mx_scl_byclust[1, i] > 0.2)) {
    if (clust_state_mx_scl_bystate[1, i] > 0.15) {
      rule_dbl_mx[1, i] <- 1
    } 
    if (clust_state_mx_scl_byclust[1, i] > 0.2){
      rule_dbl_mx[2, i] <- 1
    }
  } else if ((clust_state_mx_scl_bystate[2, i] > 0.2) | (clust_state_mx_scl_byclust[2, i] > 0.4)) {
    if (clust_state_mx_scl_bystate[2, i] > 0.2) {
      rule_dbl_mx[3, i] <- 1
    } 
    if (clust_state_mx_scl_byclust[2, i] > 0.4) {
      rule_dbl_mx[4, i] <- 1
    }
  } else {
    rule_dbl_mx[5, i] <- 1
...
rownames(rule_dbl_mx) <- c('Active - State', 'Active - Clust', 'Reactive - State', 'Reactive - Clust', 'Other - State', 'Other - Clust')
