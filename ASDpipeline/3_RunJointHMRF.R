...
viz_startmx_pval(de_zscores, gen_zscores, pval_seq, pval_seq, 'ASD',
                 paste0(figpath, 'start_numactive'), height=2000)
...
# Cycle through 4D grid and check if any parameter set violate setup
params_list <- list()
ctr <- 1
num_active <- c()
...
graph <- readRDS(paste0(folder_load, 'graph_lambda0.22.rds'))
...
for (i in 1:length(params_list)) {
  ...
  if (any(colMeans(hmrf_res$param_hist[(t-5):t, 1:3]) < (-20 + 1e-6)) | any(colMeans(hmrf_res$param_hist[(t-5):t, 4:5]) > (10 - 1e-6))) {
...
viz_mx_pval(val_mx=numactive_mx, 
            ...
            title=paste0('ASD: Num Active'), saveviz=paste0(savefigpath, "numactive.jpg"))
viz_mx_pval(val_mx=numactive_mx, 
            pval_seq_de=pval_seq, pval_seq_gen=pval_seq, null_mx = na_mx,
            title=paste0('ASD: Num Active'), saveviz=paste0(savefigpath, "numactive_na.jpg"))
...
viz_mx_pval(val_mx=numreactive_mx, 
            pval_seq_de=pval_seq, pval_seq_gen=pval_seq,
            title=paste0('ASD: Num Reactive'), saveviz=paste0(savefigpath, "numreactive.jpg"))
viz_mx_pval(val_mx=numreactive_mx, 
            pval_seq_de=pval_seq, pval_seq_gen=pval_seq, null_mx = na_mx,
            title=paste0('ASD: Num Reactive'), saveviz=paste0(savefigpath, "numreactive_na.jpg"))

## Parameter convergence ----
viz_paramconv_iter(params_list, 'ASD: 100 iters', paste0(savefigpath, "paramconv_100iters"), numiters=NULL)
...
viz_mx_pval(val_mx=overlap_mx, 
            pval_seq_de=pval_seq, pval_seq_gen=pval_seq,
            title=paste0('ASD: Num Active Overlapping'), saveviz=paste0(savefigpath, "overlap_numactive.jpg"))
...
viz_mx_pval(val_mx=overlapprop_mx, 
            pval_seq_de=pval_seq, pval_seq_gen=pval_seq,
            title=paste0('ASD: Prop Active Overlapping'), saveviz=paste0(savefigpath, "overlap_propactive.jpg"),
            null_mx = na_mx)
...
viz_mx_pval(val_mx=hamdist_mx, 
            pval_seq_de=pval_seq, pval_seq_gen=pval_seq,
            title=paste0('ASD: Avg Hamm Dist'), saveviz=paste0(savefigpath, "overlap_hamdist.jpg"))
viz_mx_pval(val_mx=hamdist_mx, 
            pval_seq_de=pval_seq, pval_seq_gen=pval_seq,
            title=paste0('ASD: Avg Hamm Dist'), saveviz=paste0(savefigpath, "overlap_hamdist_na.jpg"),
            null_mx = na_mx)

# Select Final Runs ----
# Selected final runs based on Num Active, Prop Overlapping (Active), and Hamming Dist (All State Types)
dir.create('final/states/')
final_idxs <- c(48, 49, 58, 59, 68)
