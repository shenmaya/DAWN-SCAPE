...
for (cellType in cellTypes) {
  print(cellType)
  markerGenes <- intersect(final_markergenes[[cellType]], univGenes)
  
  mx_df = data.frame(Var1=numeric(0), Var2=numeric(0), value=numeric(0), module=character(0))
  fisher_pvals <- c()
  ORs <- c()
  
  for (m_i in 1:numModules) {
    moduleGenes <- names(which(clusters == m_i))
    
    mx <- matrix(0, 2, 2)
    moduleBools <- univGenes %in% moduleGenes
    markerBools <- univGenes %in% markerGenes
    length(moduleBools) == length(markerBools)
    length(markerBools) == length(univGenes)
    ...
    mx_df_mi <- melt(mx)
    mx_df_mi$module <- rep(paste0('Module ', m_i), 4)
    
    mx_df <- rbind(mx_df, mx_df_mi)
  }
  mx_df$module <- factor(mx_df$module, paste0('Module ', 1:numModules))
  
  OR_df <- rbind(OR_df, data.frame(OR=ORs,
                                   pval=fisher_pvals,
                                   module=1:numModules,
                                   celltype=rep(cellType, numModules)))
...
# Drop modules 12-14 from significance consideration (too small)
length(cellTypes) # 7 
# 11 clusters/modules
# 7 x 11 = 77 tests
0.05/77
...
bonfpval <- 0.05
bonfpval_star <- bonfpval/(length(cellTypes) * 11)
OR_df[OR_df$module <= 11 & OR_df$pval <= bonfpval_star,]
...
## Select large clusters (>= 20) ----
# Clusters 1-11
etiol_clusts <- clust_groups$etiol_clust[clust_groups$etiol_clusts <= 11]
emerg_clusts <- clust_groups$emerg_clusts[clust_groups$emerg_clusts <= 11]
other_clusts <- clust_groups$other_clusts[clust_groups$other_clusts <= 11]
clust_order <- c(etiol_clusts, emerg_clusts, other_clusts)
...
moduleTypecolors = c('Etiological'='#E66100',
                     'Emergent'='#1A85FF',
                     'Other'='#C5B6E0')
# moduleTypecolors = c('Etiological'='#FF42A1',
#                      'Emergent'='#00A2FF',
#                      'Other'='#FFF056')
# moduleTypecolors = c('Active'='hotpink',
#                      'Reactive'='skyblue',
#                      'Other'='yellow')
...
pheatmap::pheatmap(OR_mx,
                   ...
                   width=12,height=12,
                   filename = paste0(figpath, val_method, ext, '_bonf', bonfpval, '_red2.jpg'),
                   fontsize = 20)
