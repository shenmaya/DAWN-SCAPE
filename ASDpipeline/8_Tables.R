...
gen_pvals$alias <- gen_pvals$gene

gene_df <- gen_pvals[genes, c('ensembl', 'alias')]
...
colnames(gene_df) <- c('GeneEnsembl', 'GeneSymbol', 'Cluster', 'State')
...
etiol_df <- read.csv('figs/6_Figures/GO_grouped/ASD_GO_BP0.05_etiological.csv', row.names=1)
...
emerg_df <- read.csv('figs/6_Figures/GO_grouped/ASD_GO_BP0.05_emergent.csv', row.names=1)
...
readme_df <- rbind(readme_df, data.frame('TableNumber'=c(paste0('Table S', tbl_start+7), 
                                         ...
                                         'SheetName'=c(rep('GO_Cluster', 1),
                                                       ...
                                                       rep('GO_<Etiological/Emergent/Cluster>', 2)),                                         'Variable'=c('Cluster', colnames(etiol_df)),
