...
rand_silcor_scores <- c()
label_silweight_scores <- c()
rand_silweight_scores <- c()
for (i in 1:nrow(param_grid)) {
  ...
  set.seed(42)
  for (rep in 1:100) {
    labels <- readRDS(paste0(folder_cluster, '/paramsid', i, '_resol', resol, '_k', k,
                             '/labels_resol', resol, '_k', k, '_rep', rep, '.rds'))$labels
    randlabels <- sample(labels, length(labels), replace=F)
    names(randlabels) <- names(labels)
...
results_df <- readRDS(paste0(folder_cluster, 'metrics/run_metrics_df.rds'))
...
max_index <- which.max(modua_silh_ari_01) 
...
best.params$paramid <- best.paramid
best.params
saveRDS(best.params, paste0('final/bestparams.rds'))
...
for (run in 1:reps) {
  ...
  labels <- readRDS(paste0(
    folder_cluster, '/paramsid', best.paramid, 
...
saveRDS(simprop_mx, paste0('final/simprop_mx.rds'))

simprop_mx <- readRDS(paste0('final/simprop_mx.rds'))
...
# For AD we find that all `best.k` (11) clusters have size > 5 

sim_clusters <- cutree(sim_hclust, k = best.k)
table(sim_clusters)
saveRDS(sim_clusters, paste0('final/sim_clusters_unordered.rds'))

sim_clusters <- readRDS(paste0('final/sim_clusters_unordered.rds'))
table(sim_clusters)
...
# For AD, we have clusters of size 373, 357, 231, 227,...
# We definitely want to break down 373 and 357, the clusters with 250 or less can likely be left alone

for (i in 1:best.k) {
  clusti_genes <- names(which(sim_clusters == i))
  # hclust object for cluster i
  clusti_sim_hclust <- hclust(as.dist(1-simprop_mx[clusti_genes, clusti_genes]))
  # Plot the sub-dendrogram for cluster i
  plot(clusti_sim_hclust, main = paste0("cluster ", i, " hclust from prop sim mx"), sub = "", xlab = "", cex = 0.8, labels=FALSE)
  Sys.sleep(2)
}

## Split large clusters ----
# Cluster 2 has 373 genes
clust2_genes <- names(which(sim_clusters == 2))
# hclust object for cluster 2
clust2_sim_hclust <- hclust(as.dist(1-simprop_mx[clust2_genes, clust2_genes]))
# Plot the sub-dendrogram for cluster 2
plot(clust2_sim_hclust, main = "cluster 2 hclust from prop sim mx", sub = "", xlab = "", cex = 0.8, labels=FALSE)
# Cut at a height instead of k
# rect.hclust(clust2_sim_hclust, k = 3, border = "red")
# clust2_sim_clusters <- cutree(clust2_sim_hclust, k = 3)
# rect.hclust(clust2_sim_hclust,h=0.7, border = "red")
# clust2_sim_clusters <- cutree(clust2_sim_hclust, h = 0.7)
# table(clust2_sim_clusters)

clust2_sim_clusters <- (cutreeDynamic(clust2_sim_hclust, distM=(1-simprop_mx[clust2_genes, clust2_genes]),deepSplit=2, minClusterSize=10))
names(clust2_sim_clusters) <- clust2_genes
table(clust2_sim_clusters)
plotDendroAndColors(
  clust2_sim_hclust,
  colors = labels2colors(clust2_sim_clusters),
  dendroLabels=F,
  groupLabels="Dynamic Tree Cut",
  main='Cluster 2: 373 genes'
)

# Cluster 3 has 357 genes
clust3_genes <- names(which(sim_clusters == 3))
# hclust object for cluster 3
clust3_sim_hclust <- hclust(as.dist(1-simprop_mx[clust3_genes, clust3_genes]))
# Plot the sub-dendrogram for cluster 3
plot(clust3_sim_hclust, main = "cluster 3 hclust from prop sim mx", sub = "", xlab = "", cex = 0.8, labels=FALSE)
# Visually observing the dendrogram, 3 seems like a better cut than 3 
# Highlight the k=3 clusters with rectangles
rect.hclust(clust3_sim_hclust, k = 3, border = "red")
clust3_sim_clusters <- (cutreeDynamic(clust3_sim_hclust, distM=(1-simprop_mx[clust3_genes, clust3_genes]),deepSplit=2, minClusterSize=10))
names(clust3_sim_clusters) <- clust3_genes
table(clust3_sim_clusters)
plotDendroAndColors(
  clust3_sim_hclust,
  colors = labels2colors(clust3_sim_clusters),
  dendroLabels=F,
  groupLabels="Dynamic Tree Cut",
  main='Cluster 3: 357 genes'
)

# Integrate new (sub-)clusters into main clustering
sim_clusters_split <- c()
for (gene in names(sim_clusters)) {
  if (gene %in% clust2_genes) {
    sim_clusters_split <- c(sim_clusters_split, max(sim_clusters) + clust2_sim_clusters[gene])
  } else if (gene %in% clust3_genes) {
    sim_clusters_split <- c(sim_clusters_split, max(sim_clusters) + length(unique(clust2_sim_clusters)) + clust3_sim_clusters[gene])
  } else {
