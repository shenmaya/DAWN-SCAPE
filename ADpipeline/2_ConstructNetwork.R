...
# Ensure that all fixed genetic genes are included in final 8000 genes
fixed_gen_genes <- intersect(gen_df[gen_df$fixed,]$ensembl, selgenes)
str(fixed_gen_genes)
# Select genes which are over 50% nonzero (over cells)
highnonzeroselgenes = selgenes[which(rowMeans(control_dat[selgenes,]>0)>0.5)]
str(highnonzeroselgenes) #11675
all(fixed_gen_genes %in% highnonzeroselgenes)
# Ensuring that all fixed genes are kept
highnonzeroselgenes <- unique(c(highnonzeroselgenes, fixed_gen_genes))
str(highnonzeroselgenes) #11679
all(fixed_gen_genes %in% highnonzeroselgenes)

# From previous set, select 8000 genes with largest mean values
# Ensure that all fixed genes are kept
highnonzeroselgenes_nongen <- setdiff(highnonzeroselgenes, fixed_gen_genes)
highvalselgenes = c(fixed_gen_genes,
                    highnonzeroselgenes_nongen[which.maxn(rowMeans(control_dat[highnonzeroselgenes_nongen,]),
                                                   8000-length(fixed_gen_genes))]
                    )
all(fixed_gen_genes %in% highvalselgenes)
str(highvalselgenes) # 8000
saveRDS(highvalselgenes, 'data/highvalselgenes.rds')
write.csv(data.frame('Gene'=highvalselgenes), 'data/highvalselgenes.csv')

# From previous set, select genes with either AP p-value or DE p-value below 10th percentile (respectively) or both 
...
pvals <- gen_df[match(highvalselgenes, gen_df$ensembl),]$pvals
...
# Server code
setwd('/raid6/home/myshen/gene-community-server/')
codepath <- 'mycode/'
sapply(paste0(codepath, list.files(codepath)), source)
server_path <- 'ADpipeline/2_ConstructNetwork/'
...
t1 <- Sys.time()
print(t1-t0) # Time difference of 1.449465 hours
times <- c(times, t1-t0)
...
t1 <- Sys.time()
print(t1-t0) # Time difference of 1.695134 hours
times <- c(times, t1-t0)
...
stblfolder_rd1 <- paste0(folder_rd1, 'stability_rd1/')
...
mtext("AD: Round 1 Metrics", side = 3, line = 0.5, outer = TRUE)
dev.off()
...
stblfolder_rd2 <- paste0(folder_rd2, 'stability_rd2/')
...
for (i in 1:length(metrics)) {
  ...
  if (paste0('pns_', metric) %in% names(metrics_rd2)) {
    ...
    print(clean_vals)
    smoothed <- lowess(lambdas_rd2[!is.na(clean_vals)], clean_vals[!is.na(clean_vals)])
    print(smoothed)
    lines(smoothed$x, smoothed$y, col='lightblue')
    
    abline(v=0.16, col='red', lty=2)
...
mtext("AD: Round 2 Metrics", side = 3, line = 0.5, outer = TRUE)
dev.off()
...
set.seed(42)
layout <- layout_nicely(cleangraph$net)
